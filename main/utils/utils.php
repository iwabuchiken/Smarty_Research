...
// 			return ;
		...
		perm($target, $before, $after) {
			...
			/*******************************
				table
			*******************************/
			$chars_Before = str_split($before);
			$chars_After = str_split($after);
// 			$chars_Before = explode("", $before);

			$table = array();
			
			for ($i = 0; $i < count($chars_Before); $i++) {
				
				$table[$chars_Before[$i]] = $chars_After[$i];
			...
			/*******************************
				perm
			*******************************/
			$chars_Target = str_split($target);
			
			$str_new = "";

			$len = count($chars_Target);
			
			for ($i = 0; $i < $len; $i++) {
				
				$str_new .= $table[$chars_Target[$i]];
				
			}
			
// 			printf("[%s : %d] str_orig => %s", 
// 							Utils::get_Dirname(__FILE__, CONS::$proj_Name), __LINE__, $target);
			
// 			echo "<br>"; echo "<br>";
			
// 			printf("[%s : %d] str_new => %s", 
// 							Utils::get_Dirname(__FILE__, CONS::$proj_Name), __LINE__, $str_new);
